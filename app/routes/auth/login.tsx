...
export async function loader() {
  ...
  if (user) {
    ...
    if (profile?.role === "parent") {
      throw redirect("/dashboard/parent");
...
export default function ParentLogin() {
  ...
  const [showResendButton, setShowResendButton] = useState(false); // 🔥 جديد

  useEffect(() => {
    const savedEmail = localStorage.getItem("parent_remembered_email");
    if (savedEmail) {
      setEmail(savedEmail);
      setRememberMe(true);
    }
    ...
    try {
      ...
      if (signInError) {
        ...
        if (signInError.message.includes("Email not confirmed") || 
          ...
          setShowResendButton(true); // 🔥 إظهار زر إعادة الإرسال
      ...
      // التحقق من الدور
      if (profile.role !== "parent") {
        setError("هذا الحساب ليس حساب ولي أمر");
      ...
      if (rememberMe) {
        ...
        localStorage.setItem("parent_id", profile.id);
        localStorage.setItem("parent_name", profile.full_name);
        localStorage.setItem("parent_phone", profile.phone || "");
        localStorage.setItem("parent_role", profile.role);
      } else {
        sessionStorage.setItem("parent_id", profile.id);
        sessionStorage.setItem("parent_name", profile.full_name);
        sessionStorage.setItem("parent_phone", profile.phone || "");
        sessionStorage.setItem("parent_role", profile.role);
      }
